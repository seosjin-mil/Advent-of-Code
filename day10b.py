string = """F|7.F|-F|777FJ7FF77-F777F7F|7..FF-77FF|--JJFJ7-J-F77.FF-F7F-F-FL7|--7---F.FL-7-7J.FFF7|-FFJ7.7.LF-LLJ77|F7FJJ-JFF|-.|FFF7F7..FFL7F-FJ-7-LL77
...
def valid_start_moves(a):
    (y, x) = a

    ret = []
    
    if x > 0 and string[y][x - 1] in "-LF":
        ret.append((y, x - 1))
    if x < len(string[y]) - 1 and string[y][x + 1] in "-J7":
        ret.append((y, x + 1))
    if y > 0 and string[y - 1][x] in "|7F":
        ret.append((y - 1, x))
    if y < len(string) - 1 and string[y + 1][x] in "|JL":
        ret.append((y + 1, x))
...
def valid_moves(a):
    ...
    match char:
    	case '-':
    		ret.extend([(y, x - 1), (y, x + 1)])
    	case '|':
    		ret.extend([(y - 1, x), (y + 1, x)])
    	case 'L':
    		ret.extend([(y - 1, x), (y, x + 1)])
    	case 'F':
    		ret.extend([(y + 1, x), (y, x + 1)])
    	case 'J':
    		ret.extend([(y - 1, x), (y, x - 1)])
...
visited = set()
unvisited = valid_start_moves(s_coord)

while len(unvisited):
    node_to_check = unvisited.pop(-1)
    visited.add(node_to_check)
    for i in valid_moves(node_to_check):
        if i not in visited:
            unvisited.append(i)
